...
public class E2ETransferTest extends E2EBaseTest {
	...
	void addListener() {
		...
		listener.status(simpleAccount.getAddress())
		.doOnNext(err -> logger.info("Error on recipient: {}", err))
	...
	void verifyResults() throws InterruptedException, ExecutionException {
        ...
        // return the xem
        transfer(simpleAccount, seedAccount.getAddress(), XPX.createAbsolute(BigInteger.valueOf(2)), new PlainMessage("money back guarantee"));
	}
	
	@Test
	void sendPlaintextMessage() {
		// send transaction with plain message
		transfer(seedAccount, simpleAccount.getAddress(), XPX.createAbsolute(BigInteger.valueOf(1)), new PlainMessage("java SDK plain message test"));
	}
	
	@Test
	void sendSecureMessage() {
		// send transaction with encrypted message
		SecureMessage secureMessage = SecureMessage.create(seedAccount.getKeyPair().getPrivateKey(), simpleAccount.getKeyPair().getPublicKey(), "java SDK secure message");
